...
describe ('Ontology Generator', () => {
  ...
  describe ('Builds node modules artifacts', () => {
    ...
    it ('should create from an ontology link', async () => {
        var result = await gen.generate(['https://schema.org/Person.ttl'], '1.0.0', undefined)
        expect(result).to.equal("Done!");


        expect(fs.existsSync('generated/index.ts')).to.be.true
        expect(fs.readFileSync('generated/index.ts').toString()).to.contains('Person: new LitVocabTerm(_NS(\'Person\'), undefined, true)');

        expect(fs.existsSync('generated/package.json')).to.be.true
        expect(fs.readFileSync('generated/package.json').toString()).to.contains('"name": "schema-inrupt-ext"');
    })


    it ('should be able to fully extend an ontology with multiple input files', async () => {
        var result = await gen.generate(['./test/vocabs/schema.ttl', './test/vocabs/schema-inrupt-ext.ttl'], '1.0.0');
        expect(result).to.equal("Done!");
    ...
    it ('should be able to fully extend an ontology with multiple input files and URL links', async () => {
        var result = await gen.generate(['https://schema.org/Person.ttl', './test/vocabs/schema-inrupt-ext.ttl'], '1.0.0');
        expect(result).to.equal("Done!");
    ...
    it ('should be able to extend an ontology but only creates triples from extention file', async () => {
        var result = await gen.generate(['./test/vocabs/schema.ttl'], '1.0.0', './test/vocabs/schema-inrupt-ext.ttl');
        expect(result).to.equal("Done!");
    ...
    it ('should be able to extend an ontology but only create triples from extention URL links', async () => {
        var result = await gen.generate(['./test/vocabs/schema.ttl'], '1.0.0', 'https://jholleran.inrupt.net/public/vocabs/schema-inrupt-ext.ttl');
        expect(result).to.equal("Done!");
    ...
    it ('should take in a version for the output module', async () => {
        var result = await gen.generate(['./test/vocabs/schema.ttl'], '1.0.5', './test/vocabs/schema-inrupt-ext.ttl');
        expect(result).to.equal("Done!");
